#ifndef _SDK_BASEENTITY_
#define _SDK_BASEENTITY_
...
public:
	int GetHealthNV(){
		...
		return *(int*)((uintptr_t)this + pNetVarManager->GetNVOffset("DT_BaseEntity", "m_iHealth"));
	}

	int& m_nTickBase() {
		return *(int*)((uintptr_t)this + pNetVarManager->GetNVOffset("DT_GMOD_Player", "m_nTickBase"));
	}
	...
	bool isGodded() {
		...
		Lua->PushNumber(this->entIndex());
		Lua->Call(1, 1);
		Lua->GetField(-1, "HasGodMode");
		Lua->Push(-2);
		Lua->Call(1, 1);

		bool _name = Lua->GetBool(-1);
		Lua->Pop(3);
	...
	bool isSteamFriend() {
		...
		Lua->PushNumber(this->entIndex());
		Lua->Call(1, 1);
		Lua->GetField(-1, "GetFriendStatus");
		Lua->Push(-2);
		Lua->Call(1, 1);

		std::string _name = Lua->GetString(-1);

		Lua->Pop(3);

		return (bool)(_name.find("friend") != std::string::npos);
	}
	...
	int& m_nTickBase() {
		return *(int*)((uintptr_t)this + offsets::DT_GMOD_Player::m_nTickBase);
	}
	bool IsWorld()
	{
